...
mechanism ablation (Sec.~\ref{sec:benchmarks}) shows the pre-pass moves
...
external: twenty-four single-stage flights are taken from Rogers' public
RASAero~II altitude-comparison set~\cite{rogers2015,rogers_rasaero_alt}, with
one two-stage closure (MESOS~293K) added, so the accuracy statistics are not
...
significant difference ($W=143.0$, $p=0.62$)~\cite{wilcoxon1945}. The honest
...
\begin{enumerate}
  ...
  \item \textbf{Recover the MESOS regression.} The two-stage MESOS~293K closure
    currently predicts $-6.95\%$ against a previously archived $-0.6\%$ that is
    not reproducible from the present code. Isolating and recovering this
    regression is required before the two-stage staging-and-coast model can be
