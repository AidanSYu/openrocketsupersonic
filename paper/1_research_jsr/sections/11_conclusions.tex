...
($8.10$~percentage points mean effect). The dominance of this finned-base
augmentation in the apogee error budget is examined in detail---as a distinct
research question, not a re-slice of the present integrated result---in a
companion base-drag intercomparison study~\cite{yu2026ast}.
...
\begin{enumerate}
  ...
  \item \textbf{High-Mach validation with non-reconstructed vehicles.} The
    ...
    inferred inputs.
  \item \textbf{Tighten the two-stage high-Mach closure.} The two-stage
    MESOS~293K closure carries the corpus's largest single-flight error, $-6.96\%$
    (273{,}056~ft) from the current archived code release, reproducible in
    isolation and within the $\pm10\%$ band. As the only high-Mach two-stage flight
    it couples stage separation, coast-phase aerodynamics, and Mach-4 base drag in
    a single result. Decomposing this error---and adding a second high-Mach
    two-stage flight to the corpus---is required before the two-stage
    staging-and-coast model can be presented as fully validated.
...
of the above items---particularly the transonic blending fix and the two-stage
high-Mach closure---are warmly invited through the open-source release.
