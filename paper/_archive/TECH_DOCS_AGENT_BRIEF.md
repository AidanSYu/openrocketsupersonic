...
2. **Read `paper/data/VALIDATION_MATRIX.md`** to see the current claim map and A/B/C/D evidence classes
...
4. **Basic Finner finned-vehicle benchmark** — post-Prompt-13 MAPE 11.9% vs the 8 ADA636861 multiple-fit CX0 points (Dupuis & Hathaway 1997), with 25 single-shot points archived as supporting scatter. First vehicle-level total drag validation. M 1.08-4.30. `BasicFinnerDragBenchmarkTest.java`.
...
7. **Claim-map evidence classes** — not every row is equally closed. Use `VALIDATION_MATRIX.md` as the source of truth for A/B/C/D status and do not promote B/C/D rows to external validation.
