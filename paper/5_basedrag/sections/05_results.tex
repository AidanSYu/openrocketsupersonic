...
leverage we disabled each drag mechanism in turn and re-ran 24 of the 25
corpus flights (MESOS 293K excluded; 23 single-stage plus the AeroPac 104K
two-stage closure), spanning Mach 0.54--3.46, recording the resulting shift
...
\begin{table}[htbp]
\centering
\caption{Mechanism ablation: effect of disabling each model on corpus apogee
error (mean and maximum $|\Delta|$ in percentage points, 24 of the 25 corpus
flights: MESOS 293K excluded; 23 single-stage plus the AeroPac 104K two-stage
closure). The finned-base/base-drag augmentation is the dominant apogee
