...
|---|---|---|---|---|---|
| Exact zero-lift forebody drag | `NACA RM A52H28` (local PDF: `paper/data/pdf/19930087274.pdf`) | Pure bodies of revolution already represented exactly in ORP (`cone`, `power`, `Haack`, `ogive`) | `C_DF(M)` on base-area reference | **CLOSED (2026-04-14; refreshed after Van Driest II).** Current JUnit benchmark reports aggregate `MAE = 0.029` with gate `< 0.035`; elevated percentage error is concentrated in low-drag shapes where small absolute errors dominate. | Preserve `MAE < 0.035`; document transonic cone/paraboloid bias rather than retuning broadly |
