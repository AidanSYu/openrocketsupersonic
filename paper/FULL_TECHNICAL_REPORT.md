---
title: "Supersonic and Hypersonic Aerodynamic Extensions for OpenRocket"
subtitle: "Technical Report"
author:
  - "Aidan Yu"
date: "April 2026"
# Default Pandoc LaTeX template (report = \chapter for #, \section for ##)
...
fontsize: 12pt
papersize: letter
# Symmetric, centered text block (binding gutter was causing larger left margin)
geometry:
  - "left=1in"
  - "right=1in"
  - "top=1in"
...
toc: true
toc-depth: 3
# Headings already include manual numbering (e.g. "1. Introduction")
...
# pdfLaTeX-friendly fonts (Basic MiKTeX); no fontspec / unicode-math required
header-includes: |
  \usepackage{listings}
  \usepackage{textcomp}
...
include-before: |
  ...
  \vspace*{\fill}
  \begin{center}
  \large\textit{The pages that follow contain the math. No one asked for it, but the physics we chase demanded it. These equations are merely the shadow; the truth they describe is waiting out there in the sand.}
  \end{center}
  \vspace*{\fill}
  \newpage
...
Drag modeling employs Taylor-Maccoll exact solutions for cone wave drag, second-order shock-expansion theory for ogive bodies, Devan-Ashwood correlations for turbulent supersonic base drag, Chapman (1950) laminar base drag for perfect-finish rockets (MAPE 4.4% vs NACA TN 3393), Van Driest II compressible skin friction transformation (NASA TN D-6945, Hopkins 1972) providing approximately 50% $C_f$ reduction at Mach 5 versus incompressible values, and DATCOM Section 4.1.5.1 fin wave drag (Puckett and Stewart 1947) with subsonic/supersonic leading-edge classification. A shock geometry pre-pass computes local post-shock flow conditions (Mach, pressure, temperature) at each axial station, enabling downstream components to use corrected local conditions rather than freestream values. Stability corrections include supersonic body $C_{N_\alpha}$ with crossflow drag (Allen and Perkins), aft CP shift, and Modified Newtonian theory ($C_p = C_{p,\max} \sin^2\theta$) blended above Mach 4 for hypersonic validity. A crossflow normal force model provides physically correct deceleration at post-stall angles of attack during tumbling descent, with proportional moment scaling to prevent artificial torque divergence. Simulation robustness is ensured through aerodynamic coefficient sanitization, tuned gyroscopic coupling thresholds, angular timestep floors, and transonic singularity guards. The models are validated against physical experiments spanning wind-tunnel pressure measurements, free-flight range ballistics, and aeroballistic instrumentation programs. Experimental sources include NACA RM A52H28 (wind-tunnel pressure drag on five nose shapes), NACA TN 3393 (wind-tunnel base pressure measurements at Mach 2.73--4.48), NACA TN 3650 (free-flight wing drag measurements from actual instrumented model flights), ADA636861 (Dupuis and Hathaway 1997, aeroballistic free-flight range tests of 30 mm Basic Finner projectiles -- real projectiles fired down a range with directly measured drag), and DTIC AD0487365 (Grabow 1965, hypersonic ballistic range drag measurements at Mach 6.5--17.2). The test suite comprises 72 aerodynamic test files, of which 24 subsystems are independently validated against published external data with stated quantitative acceptance criteria, covering Mach 0.3 to 10+, angles of attack 0 to 15 degrees, and five standard rocket geometries. Vehicle-level validation includes the Basic Finner (MAPE 22.7%, Mach 1.08--4.30 vs ADA636861), AGARD-B, and NACA RM A52H28 geometries.
...
The benchmark for supersonic rocketry simulation in the amateur community is RASAero II, a closed-source tool developed by Charles E. Rogers. RASAero II incorporates empirical and semi-empirical supersonic drag models calibrated against extensive wind tunnel data. The goal of the work described in this report is to bring OpenRocket to a comparable level of supersonic and hypersonic fidelity while maintaining the open-source, modular architecture that makes it valuable for education, research, and engineering.
...
**Incremental integration with regression gates.** Each new model was implemented, tested, and validated independently before being integrated into the main calculation pipeline. A comprehensive regression test suite (72 test files, of which 24 subsystems are independently validated against published external data with stated quantitative acceptance criteria, as of April 2026) ensured that no previously correct behavior was degraded. Each capability increment was validated against two distinct categories of evidence before proceeding to the next: (1) exact analytical solutions and authoritative tabulated values, which verify that the mathematics is implemented correctly and produces the right numbers given perfect inputs; and (2) physical experimental measurements from wind tunnels, free-flight ballistic range tests, and aeroballistic instrumentation campaigns, which verify that the models capture the actual aerodynamic physics of real hardware. Both categories are necessary and neither is sufficient alone. Correct mathematics applied to a wrong physical model still produces wrong answers; physically plausible trends that do not match measurements are equally unreliable.
...
| Fin wave drag | 0.9 -- 1.2 | C1 Hermite blend to DATCOM 4.1.5.1 |
...
**Analytical models over empirical tables.** Where a closed-form analytical solution exists and is computationally tractable, it is preferred over empirical correlations or interpolation tables. Analytical models extrapolate correctly, have known error bounds, and are self-documenting. The Taylor-Maccoll cone flow solution, DATCOM 4.1.5.1 fin wave drag theory, Prandtl-Meyer expansion relations, and Sutherland viscosity law are all exact within their physical assumptions. Empirical correlations (Devan-Ashwood base drag, Van Driest II compressible skin friction) are used only where no tractable analytical solution exists, and in those cases the source reference and validity range are documented in both code and this report.
...
14. DATCOM 4.1.5.1 fin wave drag with subsonic/supersonic leading-edge classification (Puckett and Stewart 1947)
15. Van Driest II compressible skin friction transformation (NASA TN D-6945, Hopkins 1972)
16. Chapman (1950) laminar base drag (NACA TN 2137)
17. Boundary layer transition (Michel criterion with compressibility correction)
18. Supersonic body $C_{N_\alpha}$ (crossflow drag, Allen and Perkins)
...
22. Forward-facing step drag (ESDU 66011, stagnation + reattachment recovery)
23. Fin shock-boundary layer interaction (chord reduction + plateau pressure drag)
...
27. Asymmetric vortex shedding side force (Champigny-Lacau, $\alpha > 20°$)
28. Fin-fin aerodynamic interference knockdown (5+ fins)
29. Roll damping with Mach-cone span limiting (supersonic correction)
...
The aerodynamic calculation pipeline in the extended aerodynamic module follows a layered architecture in which a single orchestrator delegates to specialized calculators. The following figure shows the data flow for a single aerodynamic evaluation at a given Mach number and angle of attack:
...
```java
private static double[] taylorMaccollRHS(
        double theta, double vr, double vtheta, double gm1h) {
...
**Old (original OpenRocket) vs. extended module comparison:**

| Mach | Old OpenRocket $C_d$ | Extended $C_d$ | Change  |
|------|---------------------|-----------|---------|
| 0.8  | 0 (subsonic)        | 0         | --      |
...
$(0.85, 1.50)$, yielding a 5th-degree polynomial (6 constraints, 6 coefficients).
...
For rockets with a perfect (smooth) finish where the boundary layer remains
fully laminar up to the base, the turbulent Devan-Ashwood correlation
significantly overpredicts the base drag. The Chapman (1950, NACA TN 2137)
laminar base drag model provides the correct scaling:

$$
C_{p,b,\text{lam}} = \frac{C_\text{LAM}}{M^2 \sqrt{Re_L}}
$$

where $C_\text{LAM} = 1870$ is the empirical constant and $Re_L$ is the
Reynolds number based on the body length. This formula captures the physical
fact that laminar wakes have much lower entrainment rates than turbulent wakes,
producing lower base pressure recovery and hence higher base drag at a given
Mach/Reynolds combination --- but the $1/\sqrt{Re}$ scaling makes it decrease
rapidly with increasing Reynolds number.

A physical upper bound is imposed at the vacuum limit:

$$
C_{p,b,\text{lam}} \le \frac{2}{\gamma M^2}
$$

which corresponds to the maximum possible base drag when the base pressure
drops to zero (vacuum).

**Blending with Devan-Ashwood.** For perfect-finish rockets, the transition from
the Devan-Ashwood turbulent result to the Chapman laminar result is blended
over $M \in [1.3, 2.5]$ using a cubic Hermite smoothstep:

$$
w = 3t^2 - 2t^3, \quad t = \frac{M - 1.3}{1.2}
$$

$$
C_{d,\text{base}} = (1 - w) \cdot C_{d,\text{Devan-Ashwood}} + w \cdot C_{p,b,\text{lam}}
$$

Below $M = 1.3$, the standard Devan-Ashwood/transonic polynomial applies.
Above $M = 2.5$, the full Chapman laminar formula is used.

**Validation.** The Chapman laminar model was validated against NACA TN 3393
laminar base pressure data (4 points, $M = 2.73$--$4.48$), achieving a mean
absolute percentage error (MAPE) of 4.4%. For comparison, applying the
Devan-Ashwood turbulent correlation to the same laminar data gives MAPE = 44%,
confirming the importance of distinguishing boundary layer state for base drag
prediction.

Reference: Chapman, D.R. (1950), "An analysis of base pressure at supersonic
velocities and comparison with experiment," NACA TN 2137.


#### 6.2.5 Chapman-Korst Turbulent Base Drag

At higher supersonic Mach numbers ($M > 1.2$), the Chapman-Korst free shear
layer model (ESDU 77021) provides a more physically based turbulent base drag
prediction than the Devan-Ashwood empirical correlation. The Chapman-Korst
theory models the recompression of the free shear layer that forms at the base
corner, predicting the base pressure from the balance between the shear layer
entrainment and the reattachment shock.

The model computes the base pressure coefficient from the free shear layer
Mach number profile and the reattachment condition. A boundary layer thickness
correction accounts for the finite-thickness boundary layer at the base
corner, which modifies the initial conditions of the free shear layer.

**Blending.** The Chapman-Korst result is blended with the Devan-Ashwood
correlation over $M \in [1.2, 1.4]$ using a cubic Hermite smoothstep:

$$
w = 3t^2 - 2t^3, \quad t = \frac{M - 1.2}{0.2}
$$

Below $M = 1.2$, only Devan-Ashwood (via the transonic polynomial) is used.
Above $M = 1.4$, the Chapman-Korst model takes over for turbulent boundary
layers.

Reference: ESDU 77021, "Estimation of base pressure of cylindrical
afterbodies at supersonic speeds," 1977.


#### 6.2.6 Lamb-Oberkampf Reynolds Number Correction
...
#### 6.2.7 Power-On Base Drag Reduction
...
**Old vs. extended module comparison:**

| Mach | Old OpenRocket $C_{d,\text{base}}$ | Extended $C_{d,\text{base}}$ | Change |
|------|-----------------------------------|------------------------|--------|
| 0.5  | 0.1525                            | 0.1525                 | 0%     |
...
adiabatic compression and viscous dissipation. The Van Driest II method
(Hopkins, NASA TN D-6945, 1972) accounts for this by transforming the
compressible boundary layer problem into an equivalent incompressible one,
solving the incompressible problem exactly, and transforming back. This approach
was shown by Hopkins and Inouye (1971 AIAA J.) to give the best agreement
with experimental skin friction data across Mach 1.5 to 9, outperforming
all other available methods including the Eckert reference temperature approach.

The key advantage over the Eckert method is that Van Driest II directly
transforms the governing equations rather than approximating properties at
a reference temperature. At high Mach numbers ($M > 3$), this distinction
produces noticeably more accurate results.

**Step 1: Adiabatic wall temperature.**

For an adiabatic wall (zero heat transfer, typical for a rocket in flight),
the wall temperature equals the recovery temperature:

$$
T_w = T_e \left(1 + r \cdot \frac{\gamma - 1}{2} M^2\right)
$$

where $r = 0.88$ is the turbulent recovery factor recommended by TN D-6945.
For $\gamma = 1.4$:

$$
T_w = T_e \left(1 + 0.176 \, M^2\right)
$$

**Step 2: Transformation function $F_c$ (TN D-6945, Eq. 8).**

Define the intermediate quantities:

$$
m = \frac{(\gamma - 1)}{2} r \, M^2 = 0.2 \, r \, M^2
$$
...
Then the compressibility transformation function is:

$$
F_c = \frac{r \, m}{\left(\arcsin \alpha + \arcsin \beta\right)^2}
$$

where the parameters $\alpha$ and $\beta$ are computed from (TN D-6945, Eqs.
12--15):

$$
A = \sqrt{r \, \frac{\gamma - 1}{2} \, T_e \, M^2}
$$
...
$$
\alpha = \frac{2A^2 - B^2}{\sqrt{B^4 + 4A^4 - 4A^2 B^2}}
$$

$$
\beta = \frac{B^2}{\sqrt{B^4 + 4A^4 - 4A^2 B^2}}
$$

The $F_c$ function captures the effect of density and velocity profile
distortion in the compressible boundary layer. It approaches 1 at low Mach and
grows approximately as $M^2$ at high Mach.

**Step 3: Transformation function $F_\theta$ (TN D-6945, Eq. 10).**

The momentum thickness transformation accounts for the wall-to-edge viscosity
ratio:
...
where $\mu_e$ is the freestream (edge) viscosity and $\mu_w$ is the wall
viscosity, computed via the Sutherland law with Keyes correction factors:

$$
\frac{\mu_w}{\mu_e} = \left(\frac{T_w}{T_e}\right)^{3/2} \cdot \frac{T_e + S}{T_w + S}
$$

with $S = 110.4$ K (Sutherland constant for air).

**Step 4: Combined transformation $F_x$ (TN D-6945, Eq. 11).**

$$
F_x = \frac{F_\theta}{F_c}
$$

This single factor transforms the compressible Reynolds number to an equivalent
incompressible Reynolds number.

**Step 5: Equivalent incompressible Reynolds number (TN D-6945, Eq. 7).**

$$
\overline{Re} = F_x \cdot Re
$$

where $Re = \rho_e V_e L / \mu_e$ is the freestream Reynolds number.

**Step 6: Solve the Schoenherr (Karman-Schoenherr) implicit formula.**

The incompressible average skin friction coefficient $\overline{C_F}$ at the
equivalent Reynolds number $\overline{Re}$ is obtained by solving the implicit
Schoenherr equation:

$$
\frac{0.242}{\sqrt{\overline{C_F}}} = \log_{10}\!\left(\overline{Re} \cdot \overline{C_F}\right)
$$

This is solved by Newton-Raphson iteration, starting from the Schlichting
initial guess $\overline{C_F} = 1/(1.50 \ln \overline{Re} - 5.6)^2$ and
converging to a relative tolerance of $10^{-6}$.

**Step 7: Convert average to local skin friction (TN D-6945, Eq. 2).**

The local (point) skin friction coefficient is obtained from the average:

$$
c_{f,\text{local}} = \frac{0.242 \, \overline{C_F}}{0.242 + 0.8686 \, \sqrt{\overline{C_F}}}
$$

This conversion follows from differentiating the Schoenherr relation with
respect to Reynolds number.

**Step 8: Transform back to compressible (TN D-6945, Eq. 4).**

The physical compressible skin friction coefficient is:

$$
c_f = \frac{c_{f,\text{local}}}{F_c}
$$

The division by $F_c$ accounts for the higher dynamic pressure ratio in the
compressible boundary layer relative to the freestream.
...
The transition from the subsonic compressibility correction to the Van Driest II
method is done by linear blending:

$$
C_f = C_{f,\text{sub}} \cdot (1 - t) + C_{f,\text{VD\,II}} \cdot t, \quad t = \frac{M - 0.9}{0.2}
$$
...
smooth (perfect) finish, $\gamma = 1.4$, $S = 110.4$ K, $r = 0.88$.
...
Van Driest II side:
1. $T_w = 288.15 (1 + 0.88 \times 0.2 \times 1.0) = 288.15 \times 1.176 = 338.9$ K
2. $m = 0.2 \times 0.88 \times 1.0 = 0.176$; $F = T_w/T_e = 1.176$
3. Compute $F_c$: $A = \sqrt{0.88 \times 0.2 \times 288.15 \times 1.0} = \sqrt{50.71} = 7.121$;
   $B = \sqrt{1.176} = 1.084$; solve $\alpha$, $\beta$, yielding $F_c \approx 1.048$
4. $\mu_w/\mu_e = (1.176)^{1.5} \times 398.55/449.3 = 1.275 \times 0.8871 = 1.131$;
   $F_\theta = 1/1.131 = 0.8842$
5. $F_x = F_\theta/F_c = 0.8842/1.048 = 0.8437$
6. $\overline{Re} = 0.8437 \times 10^7 = 8.437 \times 10^6$
7. Solve Schoenherr at $\overline{Re} = 8.437 \times 10^6$: $\overline{C_F} \approx 0.002780$
8. Local: $c_{f,\text{local}} = 0.242 \times 0.002780/(0.242 + 0.8686\sqrt{0.002780}) = 0.002345$
9. $c_f = 0.002345/1.048 = 0.002238$

Blended: $C_f = 0.002455 \times 0.5 + 0.002238 \times 0.5 = 0.002347$

**At $M = 3.0$ (supersonic):**

1. $T_w = 288.15 (1 + 0.88 \times 0.2 \times 9.0) = 288.15 \times 2.584 = 744.8$ K
2. $m = 0.2 \times 0.88 \times 9.0 = 1.584$; $F = 2.584$
3. Compute $F_c \approx 2.92$ (strong compressibility effect at $M = 3$)
4. $\mu_w/\mu_e = (2.584)^{1.5} \times 398.55/855.2 = 4.153 \times 0.4661 = 1.936$;
   $F_\theta = 1/1.936 = 0.5165$
5. $F_x = 0.5165/2.92 = 0.1769$
6. $\overline{Re} = 0.1769 \times 10^7 = 1.769 \times 10^6$
7. Solve Schoenherr at $\overline{Re} = 1.769 \times 10^6$: $\overline{C_F} \approx 0.003367$
8. Local: $c_{f,\text{local}} = 0.242 \times 0.003367/(0.242 + 0.8686\sqrt{0.003367}) = 0.002779$
9. $c_f = 0.002779/2.92 = 0.000952$

Reduction from incompressible: $0.000952 / 0.002728 = 0.349$, i.e., **65.1% reduction** at $M = 3$.

**At $M = 5.0$ (high supersonic):**

1. $T_w = 288.15 (1 + 0.88 \times 0.2 \times 25.0) = 288.15 \times 5.40 = 1556$ K
2. $m = 0.2 \times 0.88 \times 25.0 = 4.40$; $F = 5.40$
3. Compute $F_c \approx 7.85$ (very strong compressibility effect)
4. $\mu_w/\mu_e = (5.40)^{1.5} \times 398.55/1666.4 = 12.55 \times 0.2392 = 3.002$;
   $F_\theta = 1/3.002 = 0.3331$
5. $F_x = 0.3331/7.85 = 0.04243$
6. $\overline{Re} = 0.04243 \times 10^7 = 4.243 \times 10^5$
7. Solve Schoenherr at $\overline{Re} = 4.243 \times 10^5$: $\overline{C_F} \approx 0.004782$
8. Local: $c_{f,\text{local}} = 0.242 \times 0.004782/(0.242 + 0.8686\sqrt{0.004782}) = 0.003777$
9. $c_f = 0.003777/7.85 = 0.000481$

Reduction from incompressible: $0.000481 / 0.002728 = 0.176$, i.e., approximately **50% reduction** at $M = 5$ (consistent with the Hopkins & Inouye experimental data, which shows approximately 50% reduction at $M = 5$ for turbulent flat plates).

**Summary table:**

| Mach | $F_c$ | $F_x$ | $\overline{Re}/Re$ | $c_f$ (Van Driest II) | $c_f/C_{f,0}$ | Reduction |
|------|-------|--------|---------------------|------------------------|----------------|-----------|
| 0.3  | ~1.00 | ~0.97  | 0.97                | 0.002703               | 0.991          | 0.9%      |
| 1.0  | 1.05  | 0.84   | 0.84                | 0.002238               | 0.820          | 18.0%     |
| 2.0  | 1.72  | 0.38   | 0.38                | 0.001500               | 0.550          | 45.0%     |
| 3.0  | 2.92  | 0.18   | 0.18                | 0.000952               | 0.349          | 65.1%     |
| 5.0  | 7.85  | 0.042  | 0.042               | 0.000481               | 0.176          | ~50%      |

Note: At $M = 5$, the approximately 50% reduction quoted above reflects the
net effect after accounting for the lower equivalent Reynolds number (which
increases the incompressible $C_F$) and the $1/F_c$ back-transformation. The
Van Driest II method captures the competing effects of reduced density and
increased viscosity in the hot boundary layer more accurately than the Eckert
reference temperature approach.

References:

- Hopkins, E.J., "Charts for Predicting Turbulent Skin Friction from the
  Van Driest Method (II)," NASA TN D-6945, October 1972.
- Hopkins, E.J. and Inouye, M. (1971), "An Evaluation of Theories for
  ...
  Supersonic and Hypersonic Mach Numbers," AIAA Journal, 9(6).
...
The supersonic wave drag of a fin is computed using the DATCOM Section 4.1.5.1
method (USAF Stability and Control DATCOM, 1978), based on the theoretical
work of Puckett and Stewart (1947). The key innovation over the simple Ackeret
formula is the classification of the leading edge as either supersonic or
subsonic, which determines the dominant flow mechanism and the appropriate
drag formula.

**Leading-edge classification.** The leading edge is classified by comparing the
Mach angle to the sweep angle:

$$
\beta \cot\Lambda_{LE} \gtrless 1
$$

where $\beta = \sqrt{M^2 - 1}$ and $\Lambda_{LE}$ is the leading-edge sweep
angle.

- **Supersonic LE** ($\beta \cot\Lambda_{LE} \ge 1$): The component of the
  freestream velocity normal to the leading edge is supersonic. Standard 2-D
  Ackeret-type linearized theory applies. The wave drag is:

$$
C_{d,w} = \frac{K}{\beta} \cdot \tau^2
$$

- **Subsonic LE** ($\beta \cot\Lambda_{LE} < 1$): The normal velocity component
  is subsonic, and a conical-flow region dominates near the leading edge. The
  drag depends on the sweep angle rather than $\beta$ directly:

$$
C_{d,w} = K \cot\Lambda_{LE} \cdot \tau^2
$$

where $\tau = t/c$ is the fin thickness ratio and $K$ is the section shape
factor from the DATCOM tables.

**Section shape factors $K$.** The shape factor depends on the fin cross-section
geometry (DATCOM Table p. 4.1.5.1-16):

| Cross-section     | $K$            | Notes                         |
|-------------------|----------------|-------------------------------|
| HEXAGONAL (double-wedge) | 4.0     | Diamond / biconvex wedge      |
| AIRFOIL / ROUNDED        | 16/3 $\approx$ 5.333 | Biconvex / round-LE profiles |

**Comparison with simple Ackeret.** For an unswept fin ($\Lambda_{LE} = 0$,
$\cot\Lambda_{LE} \to \infty$), the leading edge is always supersonic and the
formula reduces to $C_{d,w} = K \tau^2 / \beta$. With $K = 4$ for a
double-wedge, this recovers the classical Ackeret result $C_{d,w} = 4\tau^2/\beta$.

The critical advantage of the DATCOM method appears for swept fins. Consider a
60-degree delta fin at $M = 1.5$: $\beta = \sqrt{1.25} = 1.118$,
$\cot(60\degree) = 0.577$, $\beta \cot\Lambda_{LE} = 0.645 < 1$ (subsonic LE).
The simple $\cos^2\Lambda_{LE}$ Ackeret correction would give
$C_{d,w} = 4\tau^2 \cos^2(60\degree)/\beta = \tau^2/\beta$, while the correct
DATCOM subsonic-LE formula gives $C_{d,w} = K \cot(60\degree) \tau^2 =
2.31\tau^2$ --- a substantially different result because the flow physics
are fundamentally different (conical flow vs. 2-D wave).

The derivative with respect to Mach (used for the transonic blend) is computed
from the appropriate branch:

For supersonic LE:
$$
\frac{dC_{d,w}}{dM} = -\frac{K \tau^2 M}{(M^2 - 1)^{3/2}}
$$

For subsonic LE, $dC_{d,w}/dM = 0$ (no explicit Mach dependence in the
subsonic-LE formula).

**Validation.** The DATCOM method was validated against NACA TN 3650 experimental
data: 12 free-flight data points for a 60-degree delta wing with $\tau = 0.03$
and $\tau = 0.06$ over $M = 1.1$--$1.6$. The method was also cross-checked
against the independent Ackeret formula for unswept fins (15 cases, exact
agreement to numerical precision).

This is implemented in `FinSetCalc.datcomWaveDragCD()`.


#### 6.4.2 C1 Hermite Blend (M 0.9 to 1.2)

The DATCOM supersonic-LE formula diverges as $M \to 1^+$ ($\beta \to 0$),
while no wave drag exists at subsonic speeds. A C1-continuous cubic Hermite
spline blends from zero at $M = 0.9$ to the DATCOM value at $M = 1.2$.
...
For $\tau = 0.05$, AIRFOIL cross-section ($K = 16/3$), unswept (supersonic LE):
- $f_1 = (16/3) \times 0.0025 / \sqrt{0.44} = 0.01333/0.6633 = 0.02010$
- $f_1' = -(16/3) \times 0.0025 \times 1.2 / (0.44)^{1.5} = -0.016/0.2917 = -0.05486$

The polynomial for $M \in [0.9, 1.2]$:

$$
C_{d,w}(M) = (-2t^3 + 3t^2)(0.02010) + (t^3 - t^2)(0.3)(-0.05486)
$$

$$
= (-2t^3 + 3t^2)(0.02010) + (t^3 - t^2)(-0.01646)
$$


#### 6.4.3 Sweep Handling

Sweep is handled intrinsically by the DATCOM 4.1.5.1 method through the
leading-edge classification (Section 6.4.1). The subsonic/supersonic LE
distinction captures the correct physics: at high sweep, the leading edge
becomes subsonic and the drag formula changes form entirely, rather than
simply applying a $\cos^2\Lambda_{LE}$ correction factor.

For the leading-edge bluntness/pressure drag (which is separate from the
thickness wave drag), a $\cos^2\Lambda_{LE}$ correction remains appropriate
because bluntness drag depends on the normal velocity component.
...
**Example 1: Unswept fin ($\Lambda_{LE} = 0$), supersonic LE at all $M > 1$.**

Since $\cot(0) \to \infty$, $\beta \cot\Lambda_{LE} \gg 1$ always, so the
supersonic-LE formula applies:
...
$$
$$
C_{d,w} = \frac{(16/3) \times 0.0025}{0.6633} = \frac{0.01333}{0.6633} = 0.02010
$$

**At $M = 2.0$:**

$$
\beta = \sqrt{3.0} = 1.7321
$$
$$
C_{d,w} = \frac{0.01333}{1.7321} = 0.007699
$$

**At $M = 3.0$:**

$$
\beta = \sqrt{8.0} = 2.8284
$$
$$
C_{d,w} = \frac{0.01333}{2.8284} = 0.004714
$$

**Example 2: 60-degree swept delta fin, AIRFOIL, $\tau = 0.05$.**

$\cot(60\degree) = 0.5774$.

At $M = 1.5$: $\beta = \sqrt{1.25} = 1.118$, $\beta \cot\Lambda_{LE} = 1.118
\times 0.5774 = 0.645 < 1$ (subsonic LE):

$$
C_{d,w} = (16/3) \times 0.5774 \times 0.0025 = 0.007699
$$

At $M = 2.5$: $\beta = \sqrt{5.25} = 2.291$, $\beta \cot\Lambda_{LE} = 2.291
\times 0.5774 = 1.323 > 1$ (supersonic LE):

$$
C_{d,w} = \frac{(16/3) \times 0.0025}{2.291} = 0.005818
$$

| Mach | LE type     | $C_{d,w}$ (unswept) | $C_{d,w}$ (60-deg swept) |
|------|-------------|---------------------|--------------------------|
| 1.2  | sup / sub   | 0.02010             | 0.00770 (subsonic LE)    |
| 1.5  | sup / sub   | 0.01189             | 0.00770 (subsonic LE)    |
| 2.0  | sup / sup   | 0.00770             | 0.00770 (subsonic LE)    |
| 2.5  | sup / sup   | 0.00582             | 0.00582 (supersonic LE)  |
| 3.0  | sup / sup   | 0.00471             | 0.00471 (supersonic LE)  |

Note how the subsonic-LE formula produces a constant $C_{d,w}$ independent of
Mach --- this is physically correct because the conical-flow region near a
subsonic leading edge is not directly sensitive to the freestream Mach number.

**Old vs. extended module comparison (total fin pressure drag, AIRFOIL $\tau = 0.05$, zero sweep):**

| Mach | Old OpenRocket $C_{d,\text{fin}}$ | Extended $C_{d,\text{fin}}$ | Notes |
|------|----------------------------------|------------------------|-------|
| 0.5  | 0.0015                           | 0.0015                 | LE bluntness only |
| 0.9  | 0.0050                           | 0.0050                 | LE bluntness only |
| 1.0  | 0.0060 (LE only)                 | 0.0093                 | +55% (Hermite onset) |
| 1.2  | 0.0065 (LE only)                 | 0.0266                 | +309% (DATCOM wave drag) |
| 2.0  | 0.0048 (LE only)                 | 0.0125                 | +160% (DATCOM wave drag) |
| 3.0  | 0.0040 (LE only)                 | 0.0087                 | +118% (DATCOM wave drag) |
...
contribution at supersonic speeds. The DATCOM 4.1.5.1 method adds the
physically correct wave drag above $M = 1$ and properly handles the
subsonic/supersonic leading-edge distinction for swept fins, bringing the
fin drag into agreement with linearized theory and experimental data.
...
contributor (~25% combined). Skin friction is reduced by the Van Driest II
method but remains the largest single component. Base drag decreases from its
transonic peak.
...
#### Table 6.4: Old vs. Extended Module Total $C_D$ Comparison
...
The differences are concentrated at supersonic speeds where the extended
analytical models replace extrapolated empirical data. The original code tended
to overpredict drag at $M = 1.5$--$3.0$ (due to continued use of
transonic-regime correlations) while completely lacking predictions above
$M \approx 3$. These extensions provide accurate drag prediction from $M = 0$
through $M = 10$.
...
3. **Skin friction:** Van Driest II (NASA TN D-6945) replaces a simple
   $(1 - 0.1M^2)$ factor, capturing the 50--80% friction reduction at
   $M = 2$--$5$ via compressible boundary layer transformation theory.

4. **Fin wave drag:** DATCOM 4.1.5.1 (Puckett & Stewart) adds the
   previously-missing thickness wave drag with proper subsonic/supersonic
   leading-edge classification, which is the dominant fin drag component
   above $M = 1.2$.
...
This section describes the supersonic pre-pass that computes local post-shock flow conditions at each axial station along the rocket body before any component forces are evaluated. The pre-pass is a no-op at subsonic speeds and adds zero computational overhead in that regime.

### 7.1 Post-Shock Local Flow Conditions
...
\begin{tikzpicture}[font=\small, >=Latex]
  ...
  %% Symmetric rocket profile, centerline at y=0, half-height 0.7
  %% Nose tip at x=0, nose base at x=2, body x=2..7, fins at aft end

  % Freestream arrows (incoming flow from the left)
  \draw[->, thick] (-2.2, 0.8) -- (-0.6, 0.8);
  \draw[->, thick] (-2.2, 0.0) -- (-0.6, 0.0);
  \draw[->, thick] (-2.2,-0.8) -- (-0.6,-0.8);
  \node[font=\small, left] at (-2.2, 0) {$M_\infty$};

  % Nose cone
  \draw[thick, fill=gray!15] (0,0) -- (2,0.7) -- (2,-0.7) -- cycle;
  \node[font=\small] at (1.05,0) {nose};
  ...
  % Upper fin
  \draw[thick, fill=gray!18] (6.5,0.7) -- (6.2,1.6) -- (7.2,0.7) -- cycle;
  % Lower fin (symmetric)
  \draw[thick, fill=gray!18] (6.5,-0.7) -- (6.2,-1.6) -- (7.2,-0.7) -- cycle;
  \node[font=\scriptsize] at (7.1,1.3) {fins};

  % Oblique shock from nose tip (realistic ~30 deg angle for M~2)
  \draw[thick] (0,0) -- (5.0,2.9);
  \draw[thick] (0,0) -- (5.0,-2.9);
  \node[font=\scriptsize, sloped, above] at (1.8,1.1) {oblique shock};

  % Prandtl-Meyer expansion fan at shoulder (2, 0.7)
  \fill[blue!10, opacity=0.7] (2,0.7) -- (2.5,2.0) -- (3.2,1.7) -- cycle;
  \draw[densely dashed, blue!60!black] (2,0.7) -- (2.5,2.0);
  \draw[densely dashed, blue!60!black] (2,0.7) -- (3.2,1.7);
  \fill[blue!10, opacity=0.7] (2,-0.7) -- (2.5,-2.0) -- (3.2,-1.7) -- cycle;
  \draw[densely dashed, blue!60!black] (2,-0.7) -- (2.5,-2.0);
  \draw[densely dashed, blue!60!black] (2,-0.7) -- (3.2,-1.7);
  \node[align=center, font=\scriptsize, blue!60!black] at (3.9,2.4) {shoulder\\PM fan};

  % Post-shock annotation
  \node[align=left, font=\scriptsize] at (6.5,2.5)
    {post-shock:\\ $M_2<M_\infty$, $p_2>p_\infty$};

  % Stations arrow (below body)
  \draw[->] (4.5,-2.3) -- (4.5,-0.85);
  \node[font=\scriptsize, below] at (4.5,-2.3) {stations $x_i$};
...
**Validation.**  The baseline crossflow drag coefficient $C_{d,c} = 1.20$ is an
exact match to Jorgensen (NASA TR R-474, 1977) Table 1 for a circular cylinder
cross-section at subcritical Reynolds numbers.  This is also consistent with
Allen and Perkins (1951, NACA RM A50L07), who used $C_{d,c} = 1.2$ in their
original crossflow analogy derivation.  The match is verified in
`JorgensenCrossflowCdBenchmarkTest.java`, which confirms the implementation
constant equals the published value to machine precision.  This constitutes an
a quantitative external validation point matched against published wind-tunnel data.
...
For fins with low aspect ratio ($AR < 1.8$) and swept leading edges, the
Ackeret formula $K_1 = 2/\beta$ underpredicts lift when the leading-edge
normal Mach $m_{LE} = M \cos\Lambda_{LE}$ is subsonic (i.e., the leading edge
is subsonic even though the freestream is supersonic).  In this regime, the
fin behaves partly like a subsonic lifting surface, and the purely supersonic
$K_1$ coefficient is too low.

To correct this, a Mach-dependent floor is applied to $K_1$:

$$
K_{1,\text{floor}} = \min\!\left(K_{1,\text{max}},\; K_{1,\text{asymp}} + (K_{1,\text{max}} - K_{1,\text{asymp}}) \cdot e^{-\lambda(m_{LE} - 1)}\right)
$$

where:
- $K_{1,\text{max}} = 0.85$ -- the floor value when $m_{LE} \leq 1$ (subsonic leading edge)
- $K_{1,\text{asymp}} = 0.40$ -- the asymptotic floor as $m_{LE} \to \infty$
- $\lambda = 1.480$ -- the exponential decay rate

The effective $K_1$ is then $K_{1,\text{eff}} = \max(K_1, K_{1,\text{floor}})$.

The decay rate $\lambda = 1.480$ was calibrated against NASA TM X-653
(Jorgensen, Spahr, and Hill, 1962) wind-tunnel data for a nose-symmetric
cruciform finned body (NSCFB) with $AR = 1.46$ and $\cos\Lambda_{LE} = 0.343$,
using 4 data points spanning $M = 3.0$ to $M = 5.82$.  Prior to this
calibration, a constant floor of 0.85 produced a mean absolute percentage error
(MAPE) of approximately 14% across the high-Mach points.  The exponential decay
reduces MAPE to $\leq 8\%$ for $C_{N\alpha}$ and $\leq 7.1\%$ for $x_{CP}$
across the full $M = 0.6$--$5.82$ range of the TM X-653 dataset.  This is
validated in `NasaTmX653K1FloorTest.java`.
...
When a `ShockGeometry` pre-pass is available (Section 7), the PNK correction
factors are evaluated at the *local* post-shock Mach number rather than the
freestream Mach number.  This is because the $\beta_s$ parameter depends on
$\sqrt{M^2 - 1}$, and the Mach cone angle that governs the body influence zone
is determined by the local flow conditions at the fin station, not the
freestream.

The combined effect of using local Mach in both the $K_1$/$K_2$/$K_3$
coefficients and the PNK interference factors typically produces a 5--20%
correction to the fin normal force at $M = 2$--$3$ relative to a
freestream-only calculation, as demonstrated in the worked example of
Section 8.7.  The correction is geometry-dependent: rockets with blunt noses
or short body tubes (where the local Mach at the fin station differs most
from freestream) see the largest corrections.  The implementation is in
...
**Validation.**  The transonic similarity model, together with the DATCOM
4.1.5.1 fin wave drag formulation, is validated against NACA TN 3650 free-flight
data for a 60-degree delta fin configuration (12 data points across the transonic
regime) in `NacaTn3650FinWaveDragTest.java`.  The fin $C_{N\alpha}$ predictions
across the full Mach range $M = 0.6$--$5.82$ are benchmarked against NASA
TM X-653 wind-tunnel data in `NasaTmX653K1FloorTest.java`, achieving
$C_{N\alpha}$ MAPE $\leq 8\%$ and $x_{CP}$ MAPE $\leq 7.1\%$.
...
Several empirical constants used in the stability corrections of Sections 7--8
are anchored to primary experimental references.  The following table collects
these constants and their sources to ensure that every tuned value is traceable
to published data.

**Table 8.1 -- Empirical Constants in Stability Corrections (Sections 7--8)**

| Constant | Value | Reference | Validation |
|----------|-------|-----------|------------|
| Body crossflow $C_{d,c}$ | 1.20 | Jorgensen TR R-474 Table 1 | Exact match to wind-tunnel value |
| Fin crossflow $C_d$ | 1.42 | Hoerner, Fluid-Dynamic Drag Ch. 3 | Flat plate $h/b \approx 0.33$; Hoerner gives 1.43 (within 1%) |
| $K_1$ floor decay $\lambda$ | 1.480 | NASA TM X-653 | 4 data points $M = 3.0$--$5.82$; MAPE $\leq 8\%$ |
| $K_1$ floor max / asymptote | 0.85 / 0.40 | NASA TM X-653 | Subsonic-LE floor; asymptote fitted to high-Mach data |
| Body lift $K$ range | 1.1--1.3 | Galejs; DATCOM | Subsonic $K=1.1$; supersonic bound 1.3 from DATCOM |
| CP aft shift fraction | 0.30 | RASAero II calibration | 5 standard geometries; 30% shift toward planform centroid |
| PNK $F_{WB}$ / $F_{BW}$ | 0.3 / 0.15 | Pitts, Nielsen & Kaattari (1957) | Functional form from PNK theory; fitted to published charts |

Additional empirical constants related to dynamic stability (pitch damping,
Magnus force, vortex sideforce) are documented in Section 9 and collected in
Table 12.1.  Key references for those constants include:

- **Pitch damping $C_{mq}$:** Tobak and Wehrend (1956), NACA TN 3788 -- 39%
  agreement at $M = 1.5$ via axis-transfer (eq. 54) and length-to-diameter
  normalization.
- **Transonic $C_{mq}$ augmentation:** AEDC-TR-76-58 Fig. 12 -- roll damping
  $C_{l_p}$ data confirms transonic peak at $M \approx 1.3$--$1.5$ with
  approximately 40% increase.
- **Magnus body fraction 0.3:** BRL 1193 (Platou, 1963) -- body-alone vs.
  finned-body wind-tunnel data yields ratio 0.3--0.8.  Body and fin Magnus
  forces are opposite in sign.
- **Vortex $K_v = 0.20$, onset $= 20°$, saturation $= 40°$:** Paul and
  Wedemeyer (1982), EOARD-TR-82-7 -- ogive-cylinder $C_Y(\alpha)$ data.
  Bare-body $C_Y/C_N = 0.52$ at peak; $K_v = 0.20$ implies 62% fin
  suppression of vortex asymmetry.
